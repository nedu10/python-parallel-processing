...
def fe_analysis(U, iK, jK, sK, F, freedofs):
    """
    Perform finite element analysis with preconditioning.
    ...
    # Preconditioner: Incomplete Cholesky factorization (LU decomposition)
    try:
        M = spilu(K)  # Incomplete LU decomposition as a preconditioner
        K_preconditioned = M.solve(K)  # Precondition the matrix
        print("Preconditioned matrix is ready.")
    except Exception as e:
        print('Warning: Failed to create preconditioner:', e)
        K_preconditioned = K  # If preconditioning fails, use the original matrix

    positive_definite = True
    try:
        spsolver = cholesky(K_preconditioned)
    except Exception as e:
    ...
    for i in range(F.shape[1]):
        if positive_definite:
            # Solve using the preconditioned system
            U[freedofs, i] = spsolver(Forces[:, i])
        else:
            # Use sparse solver if preconditioning is not used
            U[freedofs, i] = spsolve(K_preconditioned, Forces[:, i])
