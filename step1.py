...
def cholesky_gpu_sparse(K):
    """
    Perform GPU-accelerated sparse Cholesky decomposition using CuPy.

    Args:
        K (scipy.sparse.csc_matrix): Symmetric positive-definite sparse matrix.

    Returns:
        cp.ndarray: Lower triangular matrix on the GPU.
    """
    # Convert to CuPy sparse matrix
    K_gpu = cusparse.csc_matrix(K)
    # Convert to dense matrix and perform Cholesky decomposition
    K_dense_gpu = K_gpu.toarray()
    L_gpu = cp.linalg.cholesky(K_dense_gpu)
    return L_gpu
...
def fe_analysis(U, iK, jK, sK, F, freedofs):
    ...
    try:
        spsolver = cholesky_gpu_sparse(K)
